...
{
	//game_state* g = new game_state(a,x,y);
...
vector<game_state> player::possible_moves(bool enemy)
{
	...
	if(!enemy)
	{
		...
		for(int i=0;i<soldiers.size();++i)
		{
			...
			if(id==1)//we are white
			{
				...
				{
					//backward
					if((y-2) >= 0 && current_state->find_soldier(x,y-2,enemy) == -1)
					{
						game_state g = *current_state;
					...
					//positive diagonal
					if((y-2)>=0 && (x-2)>=0 && current_state->find_soldier(x-2,y-2,enemy) == -1)
					{
						game_state g = *current_state;
					...
					//negative diagonal
					if((y-2)>=0 && (x+2)< x_dim && current_state->find_soldier(x+2,y-2,enemy)==-1)
					{
						game_state g = *current_state;
						g.change_state(x,y,x+2,y-2,0,enemy);
						moves.push_back(g);
			...
			else if(id==0)//we are black 
			{
				...
				{
					//backward
					if((y+2<y_dim) && current_state->find_soldier(x,y+2,enemy) == -1)
					{
						game_state g = *current_state;
						g.change_state(x,y,x,y+2,0,enemy);
						moves.push_back(g);
					...
					//positive diagonal
					if((y+2<y_dim) && (x+2<x_dim) && current_state->find_soldier(x+2,y+2,enemy) == -1)
					{
						game_state g = *current_state;
						g.change_state(x,y,x+2,y+2,0,enemy);
						moves.push_back(g);
					...
					//negative diagonal
					if((y+2<y_dim) && (x-2>=0) && current_state->find_soldier(x-2,y+2,enemy)==-1)
					{
						game_state g = *current_state;
						g.change_state(x,y,x-2,y+2,0,enemy);
						moves.push_back(g);
		...
		for(int i=0; i<cannons.size(); i++)
		{
			...
			if(x_temp_1 >= 0 && x_temp_1 < x_dim && y_temp_1 >= 0 && y_temp_1 <y_dim && current_state->find_soldier(x_temp_1, y_temp_1, true) == -1 &&
			 current_state->find_soldier(x_temp_1, y_temp_1, false) == -1 )
			{
				game_state g = *current_state;
			...
			// cannon bombs
			if(x+3*x_par >= 0 && x+3*x_par < x_dim && y+3*y_par >= 0 && y+3*y_par < y_dim)
			{
				game_state g = *current_state;
				g.change_state(x,y,x+3*x_par,y+3*y_par,1,enemy);
				moves.push_back(g);
			}
			if(x+4*x_par >= 0 && x+4*x_par < x_dim && y+4*y_par >= 0 && y+4*y_par < y_dim)
			{
				game_state g = *current_state;
